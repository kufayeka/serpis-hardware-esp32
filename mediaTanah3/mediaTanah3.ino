...
//MQTT Topics
//------>Publish
const char *clientId = "petra/iot/krpl/serpis/esp32MediaTanah3";
const char *mediaTanah3HeartBeatTopic = "petra/iot/krpl/serpis/monitor/mediatanah3/heartbeat";
...
void maintainWifiConn(){
  ...
  client.publish(mediaTanah3HeartBeatTopic, "ON");
}
...
void readSoilSensors(){
  ...
  soilSensorPercentage3 = map(constrain(soilMoistureRealValue, soilMinReading, soilMaxReading), soilDryCal, soilWetCal, 0, 100);
...
void publishSensorsReadings(){
  if(millis() > previousPublishMillis + mqttPublishInterval){
    ...
    client.publish(mediaTanahKelembapanTanah3Topic, String(soilSensorPercentage3).c_str(), true);
  }
}
