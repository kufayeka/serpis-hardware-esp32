...
//MQTT Topics
//------>Publish
const char *clientId = "petra/iot/krpl/serpis/esp32MediaTanah2";
const char *mediaTanah2HeartBeatTopic = "petra/iot/krpl/serpis/monitor/mediatanah2/heartbeat";
...
void maintainWifiConn(){
  ...
  client.publish(mediaTanah2HeartBeatTopic, "ON");
}
...
void readSoilSensors(){
  ...
  soilSensorPercentage2 = map(constrain(soilMoistureRealValue, soilMinReading, soilMaxReading), soilDryCal, soilWetCal, 0, 100);
...
void publishSensorsReadings(){
  if(millis() > previousPublishMillis + mqttPublishInterval){
    ...
    client.publish(mediaTanahKelembapanTanah2Topic, String(soilSensorPercentage2).c_str(), true);
  }
}
